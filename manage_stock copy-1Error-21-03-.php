    ...
    try {
        if ($_POST['id']) {
            ...
            $sql = "UPDATE stocks SET part_number=:part_number, description=:description, image=:image, 
                    ...
                    stock_quantity=:stock_quantity, supplier_id=:supplier_id, location_id=:location_id, brand_id=:brand_id 
                    WHERE id=:id";
            ...
            $stmt->execute([
                ...
                'brand_id' => $brand_id, // Add brand_id
                'id' => $id
            ]);
        } else {
            // Create new stock entry
            $sql = "INSERT INTO stocks (part_number, description, image, category_id, model_id, cost, selling_price, stock_quantity, supplier_id, location_id, brand_id)
                    VALUES (:part_number, :description, :image, :category_id, :model_id, :cost, :selling_price, :stock_quantity, :supplier_id, :location_id, :brand_id)";
            $stmt = $conn->prepare($sql);
            $stmt->execute([
                'part_number' => $part_number,
                'description' => $description,
...
if (isset($_GET['view'])) {
    try {
        ...
        $sql = "
        SELECT s.*
        , c.category_name, m.model_name, su.supplier_name, l.location_name, b.brand_name 
        FROM stocks s 
...
if (!empty($filter_part_number_omit)) {
    $sql .= " AND SUBSTRING(s.part_number, 5) = :filter_part_number_omit";
}
...
if (!empty($filter_brand)) {
    $sql .= " AND s.brand_id = :filter_brand"; // Add brand filter
}
...
try {
    ...
    if (!empty($filter_brand)) {
        $stmt->bindValue(':filter_brand', $filter_brand); // Bind brand filter
    }
    $stmt->execute();
...
// Fetch categories, models, suppliers, locations, and brands for dropdowns
try {
    ...
    $locations = $conn->query("SELECT * FROM locations WHERE flag = 'active'")->fetchAll(PDO::FETCH_ASSOC);
    $brands = $conn->query("SELECT * FROM brands WHERE flag = 'active'")->fetchAll(PDO::FETCH_ASSOC); // Fetch brands
} catch (PDOException $e) {
    die("An error occurred: " . $e->getMessage());
}
        ...
        <div class="card-body">
            ...
            <div class="filter-section">
                ...
                <form method="GET" action="manage_stock.php">
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <label for="filter_part_number_omit">Part Number (Omit First 4)</label>
                            <select class="form-control" name="filter_part_number_omit" id="filter_part_number_omit">
                                <option value="">All</option>
                                <?php
                                $part_numbers_omit = $conn->query("SELECT DISTINCT part_number FROM stocks");
                                foreach ($part_numbers_omit as $part) {
                                    $part_omit = substr($part['part_number'], 4); // Omit first 4 characters
            ...
            <table class="table">
                ...
                <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php if ($crud_permissions['flag_delete'] === 'active'): ?>
                                        <a href="manage_stock.php?deactivate=<?php echo $stock['id']; ?>" class="btn btn-danger btn-sm">Deactivate</a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php } ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="11" class="text-center">No stock records found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
            ...
            <a href="manage_brands.php" class="btn btn-primary btn-sm">Manage Brands</a>
        </div>
    </div>
