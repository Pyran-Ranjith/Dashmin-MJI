...
if (isset($_POST['save_stock'])) {
    ...
    try {
        if ($_POST['id']) {
            ...
            $sql = "INSERT INTO stocks (part_number, description, image, category_id, model_id, cost, selling_price, stock_quantity, supplier_id, location_id)
                    VALUES (:part_number, :description, :image, :category_id, :model_id, :cost, :selling_price, :stock_quantity, :supplier_id, :location_id)";
            $stmt = $conn->prepare($sql);
            $stmt->execute([
                'part_number' => $part_number,
                'description' => $description,
...
if (isset($_GET['view'])) {
    try {
        ...
        $sql = "
        SELECT s.*
        , c.category_name, m.model_name, su.supplier_name, l.location_name 
        FROM stocks s 
...
try {
    ...
    $locations = $conn->query("SELECT * FROM locations WHERE flag = 'active'")->fetchAll(PDO::FETCH_ASSOC); // Fetch locations
} catch (PDOException $e) {
    die("An error occurred: " . $e->getMessage());
}
            ...
            <?php } else { ?>
                ...
                <?php if ($crud_permissions['flag_create'] === 'active' || $crud_permissions['flag_update'] === 'active'): ?>
                    <form method="POST" action="manage_stock.php">
                        ...
                        <div class="form-group row mb-3">
                            <label for="supplier_id" class="col-sm-2 col-form-label"><strong>Brand</strong></label>
                            <div class="col-sm-10">
