...
if (isset($_POST['save_stock'])) {
    ...
    $type_id = $_POST['type_id']; // Add type_id
    $cost = $_POST['cost'];
...
$filter_type = $_GET['filter_type'] ?? ''; // Add type filter
...
$sql = "SELECT 
s.*, c.category_name, m.model_name, b.brand_name, t.type_name, su.supplier_name, l.location_name 
FROM stocks s 
JOIN categories c ON s.category_id = c.id 
JOIN models m ON s.model_id = m.id 
...
try {
    ...
    $types = $conn->query("SELECT * FROM types WHERE flag = 'active' ORDER BY type_name")->fetchAll(PDO::FETCH_ASSOC); // Fetch types
} catch (PDOException $e) {
    die("An error occurred: " . $e->getMessage());
}
        ...
        <div class="card-body">
            ...
            <table class=" table-striped table table-bordered">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="13" class="text-center">No stock records found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
