...
if (isset($_POST['save_stock'])) {
    ...
    $stock_type = $_POST['stock_type']; // New field
    ...
    try {
        if ($_POST['id']) {
            ...
            $sql = "UPDATE stocks SET part_number=:part_number, description=:description, image=:image, 
                    ...
                    location_id=:location_id, rack_id=:rack_id, oem_number=:oem_number, stock_type=:stock_type 
                    WHERE id=:id";
            ...
            $stmt->execute([
                ...
                'stock_type' => $stock_type, // New field
                'id' => $id
            ]);
        } else {
            // Create new stock entry
            $sql = "INSERT INTO stocks (part_number, description, image, category_id, model_id, brand_id, type_id, 
                    cost, selling_price, stock_quantity, supplier_id, location_id, rack_id, oem_number, stock_type)
...
$filter_stock_type = $_GET['filter_stock_type'] ?? ''; // New filter
...
if (!empty($filter_stock_type)) { // New filter condition
    $sql .= " AND s.stock_type = :filter_stock_type";
}
...
try {
    ...
    if (!empty($filter_stock_type)) { // New filter binding
        $stmt->bindValue(':filter_stock_type', $filter_stock_type);
    }
    $stmt->execute();
        ...
        <div class="card-body">
                ...
                <div class="filter-area">
                    <form method="GET" action="manage_stock.php">
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <label for="filter_stock_type"><strong>Stock Type</strong></label>
                                <select class="form-control" name="filter_stock_type" id="filter_stock_type">
                                    <option value="">All</option>
                                    <option value="New" <?php echo ($filter_stock_type == 'New') ? 'selected' : ''; ?>>New</option>
                                    <option value="Old" <?php echo ($filter_stock_type == 'Old') ? 'selected' : ''; ?>>Old</option>
                                </select>
                            </div>
                        </div>
                        <div class="row mt-3">
                        ...
                        <table class="table table-bordered">
                            <tbody>
                                <?php foreach ($view_stocks as $view_stock) { ?>
                                    ...
                                    <tr>
                                        <th>Stock Type:</th>
                                        <td><?php echo $view_stock['stock_type']; ?></td>
                                    </tr>
                                    <tr>
            ...
            <?php } else { ?>
                ...
                <?php if ($crud_permissions['flag_create'] === 'active' || $crud_permissions['flag_update'] === 'active'): ?>
                    <form method="POST" action="manage_stock.php">
                        ...
                        <div class="form-group row mb-3">
                            <label for="stock_type" class="col-sm-2 col-form-label"><strong>Stock Type</strong></label>
                            <div class="col-sm-10">
                                <select class="form-control" name="stock_type" required>
                                    <option value="New" <?php echo $editing && $edit_stock['stock_type'] == 'New' ? 'selected' : ''; ?>>New</option>
                                    <option value="Old" <?php echo $editing && $edit_stock['stock_type'] == 'Old' ? 'selected' : ''; ?>>Old</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group row mb-3">
            ...
            <table class=" table-striped table table-bordered">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="16" class="text-center">No stock records found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
