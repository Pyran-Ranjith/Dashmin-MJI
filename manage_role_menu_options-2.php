...
$query = "
    ...
    -- AND roles.role_name LIKE '%$filter_role_name%' 
    ";
...
$menu_options_result_filter = $conn->query("SELECT * FROM menu_options WHERE flag = 'active'");

$roles_result = $conn->query("SELECT * FROM roles WHERE flag = 'active'");
$roles_result_filter = $conn->query("SELECT * FROM roles WHERE flag = 'active'");

$menu_tags_result = $conn->query("SELECT * FROM menu_tags WHERE flag = 'active'");
$menu_tags_result_filter = $conn->query("SELECT * FROM menu_tags WHERE flag = 'active'");
    ...
    <div class="card-body">
        <div class="card">
            ...
            <div class="card-body">
                ...
                if (isset($_GET['view'])) {
                    $id = $_GET['view'];
                    $role_menu_options_result = $conn->query("
                        SELECT role_menu_options.*, menu_options.menu_name, roles.role_name, menu_tags.tag_name 
                        FROM role_menu_options 
                        JOIN menu_options ON role_menu_options.menu_option_id = menu_options.id 
                        JOIN roles ON role_menu_options.role_id = roles.id 
                ...
                ?>
                        <hr>
                        <h3>Role Menu Ootions Details</h3>
                        <div class="card">
                            <div class="card-header">
                    ...
                    <h3>Role Menu Options</h3>
                    <div>
                        <!-- Fltering -->
                        <!-- Filter area with rounded border and colored frame -->
                        <div class="filter-area">
                            <form method="GET" action="manage_role_menu_options-2.php" id="filter">
                                <div class="row">

                                    <!-- <div class="form-group"> //In column-->
                                    <div class="col-md-3"> <!-- //In row-->
                                        <label><strong>Flter by Menu option</strong></label>
                                        <select class="form-control" name="filter_menu_name">
                                            <option value="">Select Menu option</option>
                                            <?php while ($menu_options = $menu_options_result_filter->fetch(PDO::FETCH_ASSOC)) { ?>
                                                <option value="<?php echo $menu_options['menu_name']; ?>" <?php //if ($roles['id'] == $role_id) echo 'selected'; 
                                                                                                            ?>>
                                                    <?php echo $menu_options['menu_name']; ?>
                                                </option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-md-3">
                                        <label><strong>Flter by Role</strong></label>
                                        <select class="form-control" name="filter_role_name">
                                            <option value="">Select Role</option>
                                            <?php while ($roles = $roles_result_filter->fetch(PDO::FETCH_ASSOC)) { ?>
                                                <option value="<?php echo $roles['role_name']; ?>" <?php //if ($roles['id'] == $role_id) echo 'selected'; 
                                                                                                    ?>>
                                    ...
                                    <div class="col-md-3">
                                        <label><strong>Flter by Menu Tag</strong></label>
                                        <select class="form-control" name="filter_tag_name">
                                            <option value="">Select Tag Name</option>
                                            <?php while ($menu_tags = $menu_tags_result_filter->fetch(PDO::FETCH_ASSOC)) { ?>
                                                <option value="<?php echo $menu_tags['tag_name']; ?>" <?php //if ($roles['id'] == $role_id) echo 'selected'; 
                                                                                                        ?>>
                                                    <?php echo $menu_tags['tag_name']; ?>
                                                </option>
                                            <?php } ?>
                                        </select>
                                    </div>
