...
function generateLocationCode($floor, $rack_number, $row_number, $column_number, $side) {
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['add_rack'])) {
            ...
            // Add new rack
            $stmt = $conn->prepare("INSERT INTO racks 
                (location_id, floor, rack_number, row_number, column_number, side, description, location_code) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $_POST['location_id'],
                $_POST['floor'],
            ...
            // Generate updated location code
            $location_code = generateLocationCode(
                $_POST['floor'],
                $_POST['rack_number'],
                $_POST['row_number'],
            ...
            // Soft delete rack (set status to inactive)
            $stmt = $conn->prepare("UPDATE racks SET status = 'inactive' WHERE id = ?");
            $stmt->execute([$_POST['rack_id']]);
            $_SESSION['message'] = "Rack deactivated successfully!";
        } elseif (isset($_POST['activate_rack'])) {
            // Reactivate rack
            $stmt = $conn->prepare("UPDATE racks SET status = 'active' WHERE id = ?");
            $stmt->execute([$_POST['rack_id']]);
            $_SESSION['message'] = "Rack activated successfully!";
        }
    ...
    $query = "SELECT r.*, l.location_name,
              ...
              LEFT JOIN fifo_queue1 fq ON r.id = fq.rack_id AND fq.is_processed = 0
    ...
    $query .= " GROUP BY r.id ORDER BY r.location_id, r.floor, r.rack_number, r.row_number, r.column_number, r.side";
    ...
    // Get all locations
    $locations = $conn->query("SELECT id, location_name FROM locations WHERE flag = 'active' ORDER BY location_name")->fetchAll(PDO::FETCH_ASSOC);
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        <?php if (empty($racks)): ?>
                            <tr>
                                <td colspan="6" class="text-center">No racks found</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($racks as $rack): ?>
                                <tr>
                                    <td><?= htmlspecialchars($rack['location_name']) ?></td>
                                    <td><?= $rack['location_code'] ?></td>
                                    <td><?= htmlspecialchars($rack['description']) ?></td>
                                    <td class="text-center">
                                        <?= $rack['assigned_parts'] ?>
                                        <?php if ($rack['part_numbers']): ?>
                                            <br><small><?= htmlspecialchars($rack['part_numbers']) ?></small>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <span class="badge bg-<?= $rack['status'] == 'active' ? 'success' : 'secondary' ?>">
                                            <?= ucfirst($rack['status']) ?>
                                        </span>
                                    </td>
                                    <td>
                                        <?php if ($rack['status'] == 'active'): ?>
                                            <button type="button" class="btn btn-sm btn-primary edit-rack" 
                                                    data-id="<?= $rack['id'] ?>"
                                                    data-location="<?= $rack['location_id'] ?>"
                                            ...
                                            <form method="post" style="display:inline;">
                                                ...
                                                <button type="submit" name="delete_rack" class="btn btn-sm btn-warning" 
                                                        onclick="return confirm('Are you sure you want to deactivate this rack?')">
                                        ...
                                        <?php else: ?>
                                            <form method="post" style="display:inline;">
                                                <input type="hidden" name="rack_id" value="<?= $rack['id'] ?>">
                                                <button type="submit" name="activate_rack" class="btn btn-sm btn-success">
