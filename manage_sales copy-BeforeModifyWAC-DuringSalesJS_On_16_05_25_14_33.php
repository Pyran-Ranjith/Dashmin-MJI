...
<script>
 // Add WAC display functionality
document.addEventListener('DOMContentLoaded', function() {
    const stockSelect = document.querySelector('select[name="stock_id"]');
    const wacDisplay = document.createElement('div');
    wacDisplay.className = 'alert alert-info mt-3';
    wacDisplay.innerHTML = '<strong>WAC:</strong> Select a part number to see WAC';
    stockSelect.parentNode.appendChild(wacDisplay);
    ...
    stockSelect.addEventListener('change', function() {
        ...
        // Fetch WAC via AJAX
        fetch(`get_wac.php?part_id=${partId}`)
        // fetch(`get_wac.php?part_id=36`)
            .then(response => response.json())
            .then(data => {
                wacDisplay.innerHTML = `
                    <strong>WAC Information:</strong>
                    <ul>
                        <li><strong>Current WAC:</strong> ${data.current_wac ? data.current_wac.toFixed(2) : 'N/A'}</li>
                        <li><strong>Historical WAC:</strong> ${data.historical_wac ? data.historical_wac.toFixed(2) : 'N/A'}</li>
                    </ul>
                    <div class="form-group mt-2">
                        <label><strong>Selling Price (X)</strong></label>
                        <input type="number" class="form-control" name="selling_price" placeholder="Enter selling price" step="0.01">
                    </div>
                `;
            })
            .catch(error => {
                wacDisplay.innerHTML = '<strong>Error:</strong> Could not fetch WAC data';
