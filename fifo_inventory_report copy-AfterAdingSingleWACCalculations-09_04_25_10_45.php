...
function calculateWeightedAverage($conn, $part_id) {
    try {
        $stmt = $conn->prepare("SELECT 
                                SUM(quantity * cost) / NULLIF(SUM(quantity), 0) AS avg_cost,
        ...
        return ['avg_cost' => 0, 'total_quantity' => 0];
    }
}
...
try {
    ...
    // Calculate weighted averages for all parts if no specific part is selected
    $weighted_averages = [];
    if (empty($part_id)) {
        $stmt = $conn->query("
            SELECT part_id, 
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $weighted_averages[$row['part_id']] = $row;
        }
    }
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <?php if (!empty($part_id)): ?>
                <?php 
                $avg_data = calculateWeightedAverage($conn, $part_id);
                $total_value = $avg_data['avg_cost'] * $avg_data['total_quantity'];
                ?>
                <div class="text-end">
                    <div class="fw-bold">Weighted Avg Cost: <?= number_format($avg_data['avg_cost'], 4) ?></div>
                    <div>Total Value: <?= number_format($total_value, 2) ?></div>
                </div>
            <?php endif; ?>
        </div>
        <div class="card-body">
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        <?php if (empty($inventory)): ?>
                            <tr>
                                <td colspan="<?= empty($part_id) ? 9 : 8 ?>" class="text-center">No inventory items found matching your criteria</td>
                            </tr>
                        <?php else: ?>
                            <?php 
                            $current_part = null;
                            foreach ($inventory as $item): 
                                if ($current_part !== $item['part_id']):
                                    ...
                                    $avg_data = empty($part_id) ? 
                                        ($weighted_averages[$item['part_id']] ?? ['avg_cost' => 0]) : 
                                        calculateWeightedAverage($conn, $item['part_id']);
                                endif;
                            ?>
                                <tr>
                                    ...
                                    <td><?= number_format($item['cost'], 4) ?></td>
                                    ...
                                    <?php if (empty($part_id)): ?>
                                        <td><?= number_format($avg_data['avg_cost'], 4) ?></td>
                                    <?php endif; ?>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
