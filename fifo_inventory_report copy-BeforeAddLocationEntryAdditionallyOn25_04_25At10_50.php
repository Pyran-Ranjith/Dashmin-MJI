...
try {
    ...
    $query = "SELECT 
                ...
                r.column_number,
                r.side,
                CONCAT('F', r.floor, '-R', r.rack_number, '-RW', r.row_number, '-C', r.column_number, '-', UPPER(LEFT(r.side, 1))) AS rack_location
              FROM fifo_queue1 fq
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        <?php if (empty($inventory)): ?>
                            <tr>
                                <td colspan="10" class="text-center">No inventory items found matching your criteria</td>
                            </tr>
                        <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($item['rack_number']): ?>
                                            F<?= $item['floor'] ?>-R<?= $item['rack_number'] ?>-RW<?= $item['row_number'] ?>-C<?= $item['column_number'] ?>-<?= strtoupper(substr($item['side'], 0, 1)) ?>
                                        <?php else: ?>
                                            N/A
                                        <?php endif; ?>
                                    </td>
